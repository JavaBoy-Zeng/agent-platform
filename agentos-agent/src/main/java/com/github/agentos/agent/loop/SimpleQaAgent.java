...
public final class SimpleQaAgent implements Agent, AgentLoop {
    ...
    private static final String DEFAULT_INSTRUCTION = """
            ...
            关于你的运行位置：你是运行在用户本机上的 Agent 平台的一部分，平台具备读写本机
            文件、执行命令、启动本机程序与联网抓取的工具能力。当前这一轮走的是不带工具的
            直答通道，因此你本轮无法调用它们，但这只是本轮通道的限制。
            禁止声称自己是云端服务、禁止声称无法访问用户的电脑或文件系统。

            如果问题需要实时信息、文件读写或其他工具才能回答，直接说明这一轮是直答通道、
            需要走带工具的任务通道，并邀请用户把它作为一个明确任务再说一次
            （例如“把上面的内容写成 markdown 文件保存到桌面”），不要给出让用户手工操作的
            替代步骤。

            关于会话记忆：本轮消息里在当前输入之前的 user/assistant 消息，是本会话真实
            发生过的历史轮次。必须据此解析指代与省略，并直接采用用户已经陈述过的稳定事实
