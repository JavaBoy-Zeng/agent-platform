 ...
 *   <li>{@code sequential} → {@link SequentialAgent}（引用已构建的子 Agent）</li>
 * </ul>
 *
 * <p>构建分两阶段：先构建 specialist（叶子），再构建 sequential（组合），
 * 确保子 Agent 引用可以被解析。</p>
 */
public final class AgentFactory {
     ...
     * 按声明顺序构建全部 Agent。
     *
     * <p>specialist 先构建（叶子节点），sequential 后构建（组合节点），
     * 同一声明中的 sequential 可引用前面已构建的 specialist。</p>
     *
