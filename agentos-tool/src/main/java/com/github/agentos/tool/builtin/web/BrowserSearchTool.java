...
/**
 * 调用 SearXNG 服务的网页浏览器搜索工具，作为系统主要的网页搜索入口。
 *
 * <p>SearXNG 是自部署的开源元搜索引擎（Docker 镜像 {@code searxng/searxng:latest}），
 * 聚合 Google、Bing、DuckDuckGo 等引擎结果且无需 API Key。请求格式：
 * {@code GET <endpoint>?q=<query>&format=json}，需在 SearXNG settings 中启用 JSON 输出。
 * 返回带标题、URL 与摘要的结构化搜索结果。</p>
 */
public final class BrowserSearchTool implements AgentTool {
    ...
    /** 创建 SearXNG 网页浏览器搜索工具。 */
    public BrowserSearchTool(
    ...
    public ToolResult execute(ToolContext context, ToolCall call) {
        ...
        try {
            ...
            return ToolResult.failure(
                    ...
                    "web search request failed: " + safeMessage(exception));
        }
    ...
    /** ConnectException 等异常的 message 可能为 null，回退到类名保证错误信息可诊断。 */
