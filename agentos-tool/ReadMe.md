...
| `browser_search` | 通过自部署 SearXNG 按关键词发现候选页面，只返回标题、URL 和摘要，不读取正文。 |
| `web_fetch` | 读取一个已知 HTTP(S) URL 的单页正文；不搜索、不遍历站点，受响应大小和输出长度限制。 |
| `web_crawl` | 通过 Firecrawl 从入口遍历多个页面并返回有界 Markdown 正文，适合文档站等多页资料。 |
...
`web_crawl` 使用 `agentos.tools.firecrawl` 配置。设置 `FIRECRAWL_API_KEY` 后会自动注册；
调用自部署且未启用认证的 Firecrawl 时，设置 `enabled=true` 与 `FIRECRAWL_API_URL`。
该工具限制单次页面数量与输出字符数，并会响应 Invocation 取消和任务轮询总超时。
